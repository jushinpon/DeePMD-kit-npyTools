...
use Data::Dumper;
use POSIX;
use lib '/opt/QElmp_tools/';
use elements;#all setting package
use Cwd;

#my @largefiles = `find ./ -name "*" -type f|grep -v "raw0"|grep -v "json"|grep -v ".pl"`;#|*.json'`;#|*.pl'`;#|egrep -v 'type.raw|*.json'`;#|grep -v '*.json'`;
my @rm_files = `find ./ -name "*" -type f|grep "raw0"`;# files need to remove
map { s/^\s+|\s+$//g; } @rm_files; 
